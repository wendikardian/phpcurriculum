...
<body>
    ...
    <?php if ($totalCostIncludeFee > 0) :  ?>
        <p>Desciption : To Carry out road repairs with a total length of <?= $totalLength; ?> meters, Perumahan Graha sentosa must prepare a total cost of Rp. <?= $totalCostIncludeFee ?>.</p>
        <p>Budget : Rp. <?= $budget ?></p>

        <?php
        if ($isCashReady) : ?>
            <?php
            if ($totalCostIncludeFee < $budget) : ?>
                <p>Fund Status : The budget is sufficient, the souce of self-supporting funds from the resident of Perumahan Graha Sentosa only.</p>
                <p>
                    Project status : <span style='color: green;'>The project will be implemented soon !</span>
                </p>

            <?php elseif (($budget * 2) > $totalCostIncludeFee) : ?>
                <p>Fund Status : Lack of funds - to cover the lack of funcds, a fund-raising bazaar was held.</p>
                <p>Project Status : <span style='color: red;'>Will be postponed, until funds are available</span></p>
            <?php else : ?>
                <p>Fund Status : Lack of funds - apply for financial assistance to the local government.</p>
                <p>Project Status : <span style='color: red;'>Will be postponed, until funds are available</span></p>
            <?php endif; ?>
